...
class decode_Sampler_mol(object):
    ...
    def sample(self, strat='controlled', sample_M=20):
        ...
        self.sampling_fn = load_sampling_fn(self.configt, self.config.sampler, self.config.sample, self.device, strat=strat)
        ...
        if strat=='controlled':
            ...
            return x, adj, x_mid, adj_mid

    @torch.no_grad()
    def controlled_decode(self, gen_batch_num=1, sample_M=20):
        # samples = []
        # value_func_preds = []
        reward_model_preds = []
        for i in range(gen_batch_num):
            x, adj = self.sample(strat='controlled', sample_M=sample_M)
    ...
    def qed_reward(self, x, adj):
        ...
        qed_scores = [QED.qed(mol) for mol in gen_mols]
